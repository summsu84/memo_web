var obj_NgApp = angular.module('app_trace', ['ngRateIt', 'ui.bootstrap']);

obj_NgApp.controller('ctr_trace', function ($scope, $http, $document, $window) {
    ...
    $scope.searchDetailHanlder = function(idx) {
        ...
        var dataObj = {};
        addDataObj(jQuery, dataObj, "trace_id", $scope.test_cols[idx]._id);

        $http.post(ctrUrl, dataObj).success(function (returnData) {
            $scope.traceDtl = returnData.traceDtl;
    ...
    $scope.completetrace = function(idx, dueDate, desc) {

        var ctrUrl = baseUrl + '/traceDone';
        var dataObj = {};
        addDataObj(jQuery, dataObj, "sel_id", $scope.sel_id);
        addDataObj(jQuery, dataObj, "dtl_id", $scope.traceDtl[idx]._id);
